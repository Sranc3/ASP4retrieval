...
from modules.module_cross import MLP
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if hasattr(task_config, "eval_method"):
            self.sim_header = task_config.sim_header
            self.eval_method = self.task_config.eval_method
    ...
    def programme_1(self, sequence_output, visual_output, attention_mask, video_mask, sim_header='seqTransf'):
        ...
        for i in range(seq_len):
            ...
            x_m = torch.sqrt(torch.sum(x * x))
            y_m = torch.sqrt(torch.sum(y * y, dim=-1))
            scores = torch.matmul(y, x.transpose(-1, -2)).squeeze(-1) / (x_m * y_m)  # 4, 1, 64
            most_smi = torch.argmax(scores, dim=-1)
            y = y[range(y.shape[0]), most_smi].unsqueeze(1)
    ...
    def programme_5(self, sequence_output, visual_output, attention_mask, video_mask, sim_header='seqTransf'):
        ...
        for i in range(seq_len):
            x = sequence_output[i]
            y = visual_output
            z = visual_output_original
            x_m = torch.sqrt(torch.sum(x * x))
            y_m = torch.sqrt(torch.sum(y * y, dim=-1))
            z_m = torch.sqrt(torch.sum(z * z, dim=-1))
            ...
            scores = torch.matmul(z, x.transpose(-1, -2)).squeeze(-1) / (x_m * z_m)  # 4, 1, 64
            most_smi = torch.argmax(scores, dim=-1)
            z = z[range(z.shape[0]), most_smi].unsqueeze(1)
    ...
    def temporal_grounding(self, sequence_output, visual_output, attention_mask, video_mask, sim_header='seqTransf'):
        ...
        for i in range(seq_len):
            x = sequence_output[i]  #1*512
            y = visual_output  #4*64*512
            # net = nn.Linear(512, 1)
            # net.to('cuda:0')
            # net(y)
            #
            # net = nn.Conv2d(16, 1, kernel_size=1, stride=1)
            # net.to('cuda:0')
            # net(y.unsqueeze(0))
            l = y.size()[0]
            mul_feature = x * y
            add_feature = x + y
            #print((x.repeat(l,64,1)).size(),y.size(),l)
            cat_feature = torch.cat((x.repeat(l,64,1), y), 2)
            cross_modal_vec = torch.cat((mul_feature, add_feature, cat_feature), 2)  #4*64*2048

            #print("x:{},y:{},cross_modal_vec:{}".format(x.size(), y.size(), cross_modal_vec.size()))
            # out = cross_modal_vec.unsqueeze(0).permute(0, 3, 1, 2)  # match conv op
            # print(out.size())
            # print(str(visual_output.device))
            # if str(visual_output.device) != 'cuda:0':
            #     print(visual_output.device.type)
            #     print("visual_output.device{}".format(visual_output.device))
            #     print("parameters:{}".format(self.fc1.parameters()))
            #     print("device{}".format(next(self.fc1.parameters()).device))

            # print(self.fc)
            # print(self.transformerClip)
            # net = nn.Conv2d(2048, 1, kernel_size=1, stride=1)
            # net.to('cuda:0')
            # X = net(out)    1*2048*4*64 --> 1*1*4*64   -->1*4*64*1
            # print(type(out))
            # print(type(torch.randn([3,3])))
            # net = nn.Linear(3,1)
            # net(torch.randn([3,3]))
            # print("cross_modal_vec{}".format(cross_modal_vec.size()))
            out = self.fc(cross_modal_vec)
            # print("out.size{}".format(out.size()))
            #print("out1:{}".format(out.size()))
            out = out.squeeze(2) # 4 * 64
            #print("out2:{}".format(out.size()))
            most_smi = torch.argmax(out, dim=-1) # 4 * 1
            #print("most_smi:{}".format(most_smi.size()))
            y = y[range(y.shape[0]), most_smi].unsqueeze(1)
    ...
    def double_flow(self, sequence_output, visual_output, attention_mask, video_mask, sim_header='seqTransf'):
       
        sequence_output, visual_output = sequence_output.contiguous(), visual_output.contiguous()  
        visual_output_original = visual_output
        sequence_output = sequence_output / sequence_output.norm(dim=-1, keepdim=True)
        minus_visual_output = visual_output

        a = minus_visual_output.shape[0]
        c = minus_visual_output.shape[2]
        device = minus_visual_output.device
        zeros = torch.zeros([a, 1, c], dtype=torch.float32).cuda(device)
        minus_visual_output = minus_visual_output - torch.cat((zeros, minus_visual_output), 1)[:, :-1, :]
        #print(minus_visual_output.norm(dim=-1, keepdim=True).shape, minus_visual_output.norm(dim=-1, keepdim=True))
        # print((minus_visual_output.norm(dim=-1, keepdim=True) + torch.tensor((0.0001))), (minus_visual_output.norm(dim=-1, keepdim=True) + torch.tensor((0.0001))).shape)
        # sys.exit()
        minus_visual_output = minus_visual_output / (minus_visual_output.norm(dim=-1, keepdim=True) + torch.tensor((0.0001)))
        ...
        seq_length = minus_visual_output.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=minus_visual_output.device)
        position_ids = position_ids.unsqueeze(0).expand(minus_visual_output.size(0), -1)
        frame_position_embeddings = self.frame_position_embeddings(position_ids)
        minus_visual_output = minus_visual_output + frame_position_embeddings
        extended_video_mask = (1.0 - video_mask.unsqueeze(1)) * -1000000.0
        extended_video_mask = extended_video_mask.expand(-1, video_mask.size(1), -1)
        minus_visual_output = minus_visual_output.permute(1, 0, 2)  # NLD -> LND
        minus_visual_output = self.transformerClip(minus_visual_output, extended_video_mask)
        minus_visual_output = minus_visual_output.permute(1, 0, 2)  # LND -> NLD
        ...
        for i in range(seq_len):
            x = sequence_output[i]
            y = visual_output
            z = minus_visual_output
            x_m = torch.sqrt(torch.sum(x * x))
            y_m = torch.sqrt(torch.sum(y * y, dim=-1))
            z_m = torch.sqrt(torch.sum(y * y, dim=-1))
            scores = torch.matmul(y, x.transpose(-1, -2)).squeeze(-1) / (x_m * y_m)
            most_smi = torch.argmax(scores, dim=-1)
            y = y[range(y.shape[0]), most_smi].unsqueeze(1)

            scores = torch.matmul(z, x.transpose(-1, -2)).squeeze(-1) / (x_m * z_m)
            most_smi = torch.argmax(scores, dim=-1)
            z = z[range(z.shape[0]), most_smi].unsqueeze(1)
            # sim_matrix = nn.functional.softmax(scores, dim=-1)
            # y = sim_matrix.unsqueeze(-1).repeat(1,1,512) * y
            w1 = 0.6
            w2 = 0.2
            after_select = visual_output_original + y * w1 + z * w2
    ...
    def get_similarity_logits(self, sequence_output, visual_output, attention_mask, video_mask, shaped=False,
        ...
        if loose_type:
            ...
            if eval == 'myeval':
                retrieve_logits = self.ASP_logits(sequence_output, visual_output, attention_mask, video_mask,
                                                 sim_header=self.sim_header)
            elif eval == 'programme_1':
                retrieve_logits = self.programme_1(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'programme_2':
                retrieve_logits = self.programme_2(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'programme_3':
                retrieve_logits = self.programme_3(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'programme_4':
                retrieve_logits = self.programme_4(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'programme_5':
                retrieve_logits = self.programme_5(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'temporal_grounding':
                retrieve_logits = self.temporal_grounding(sequence_output, visual_output, attention_mask, video_mask,
                                                   sim_header=self.sim_header)
            elif eval == 'temporal_difference':
                retrieve_logits = self.temporal_difference(sequence_output, visual_output, attention_mask, video_mask,
                                                          sim_header=self.sim_header)
            elif eval == 'double_max':
                retrieve_logits = self.double_max(sequence_output, visual_output, attention_mask, video_mask,
                                                          sim_header=self.sim_header)
            elif eval == 'double_flow':
                retrieve_logits = self.double_flow(sequence_output, visual_output, attention_mask, video_mask,
                                                          sim_header=self.sim_header)
            else:
